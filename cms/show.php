...
include_once("php_includes/include_student_column.php");
   ...
   $sql = "SELECT DISTINCT sid,cid,request_type,purpose,request_date,status FROM certificate JOIN student_certificate_requests1 ON certificate.cid=student_certificate_requests1.c_id WHERE sid='".$sid."'";
  // mysqli_select_db('cms');
...
<tbody>
  ...
  $retval = mysqli_query($conn, $sql );
   ...
   while($row = mysqli_fetch_ASSOC($retval/*, MYSQLi_ASSOC*/))
   {
...
?>
    ...
    <script sc="js/yii_002.js"></script>
<script src="js/jquery_002.js"></script>
<script type="text/javascript">jQuery(document).ready(fnction () {
...
jQuery(document).on('ubmit', "#w0 form[data-pjax]", function (event) {jQuery.pjax.submit(event, '#w0', {"push":false,"replace":false,"timeout":1000,"scrollTo":false});});
});</script>    
