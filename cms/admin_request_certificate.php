<?php
   ...
   $sql = "SELECT distinct sid,cid,request_type,purpose,request_date,status FROM certificate JOIN student_certificate_requests1 on certificate.cid=student_certificate_requeosts1.c_id WHERE status='Pending'";
...
<?php
   ...
   if(! $retval )
   {
      die('No Pending Certificate: ' . mysqli_error($conn));
   }
